...
def create_heatmap(image_no,df,categories=["hard negative","mitotic figure"]):
  bin_mask = create_mask(image_no,df,categories)
  itkimg = sitk.GetImageFromArray(bin_mask)
  sdmap = sitk.SignedDanielssonDistanceMap(itkimg,useImageSpacing=True,insideIsPositive=True)
  sdmap_array = sitk.GetArrayFromImage(sdmap)
  return sdmap_array
