...
from MIDOG.Code.neural_style_transfer_processing import preprocess, deprocess
...
class MIDOG_Images_NST(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = style_transfer(image_path,style_path,self.model,self.device,epochs=self.nst_epochs,
                               print_out=False,show_every=self.nst_epochs-1,show_images=False,max_size=512)[-1]
...
def create_data_loaders_nst(train_ids,valid_ids,model,device,nst_epochs=101,image_transform=None,mask_transform=None,train_transform=None,test_ids=None,shuffle_valid=True,
                        normalise="ImageNet",image_dir=None,mask_dir=None,get_mask_name=get_mask_name,image_end_path=image_end_path,mask_end_path=mask_end_path):
...
class NST(object):
    def __init__(self, style_images,model,device):
    ...
    def __call__(self, sample):
        ...
        for image in images:
          ...
          styled_image=style_transfer(image,style_image,self.model,self.device,print_out=False,show_every=100,show_images=False,input_is_path=False)[-1]
